...
const AddNewGroup = () => {
  ...
  // Track the last auto-generated name to know if we should update/clear it
  const lastAutoGeneratedNameRef = useRef<string>("");
  ...
  // Only auto-fill when coming from selector (params.selectedFriendId exists)
  const selectedFriends = useMemo(() => {
    return friends.filter((friend) => selectedMemberIds.includes(friend._id));
  }, [selectedMemberIds, friends]);

  useEffect(() => {
    // Need authData name to auto-generate
    if (!authData?.name || !currentUserId) return;

    // Get total member count (including current user)
    const totalMembers = selectedMemberIds.length;

    // Get member IDs excluding current user
    const memberIdsExcludingCurrent = selectedMemberIds.filter(
      (id) => id !== currentUserId
    );

    // Get friends excluding current user from the friends list
    const friendsExcludingCurrent = friends.filter((friend) =>
      memberIdsExcludingCurrent.includes(friend._id)
    );

    // Use functional update to access current groupName without adding it to dependencies
    setGroupName((currentName) => {
      // Check if current name matches the auto-generated pattern
      const autoGeneratedPattern = new RegExp(
        `^${authData.name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")} \\| .+$`
      );
      const isAutoGenerated = autoGeneratedPattern.test(currentName);

      // Auto-generate name only when exactly 2 members total (self + 1 friend)
      if (totalMembers === 2 && friendsExcludingCurrent.length === 1) {
        // Exactly 2 members (self + 1 friend): set auto-generated name
        const friendName = friendsExcludingCurrent[0].name;
        const autoFillName = `${authData.name} | ${friendName}`;

        // Update if name is empty, matches auto-generated pattern, or matches last auto-generated
        if (
          !currentName.trim() ||
          isAutoGenerated ||
          currentName === lastAutoGeneratedNameRef.current
        ) {
          lastAutoGeneratedNameRef.current = autoFillName;
          return autoFillName;
        }
        // User manually edited, preserve their input
        return currentName;
      } else {
        // More than 2 members, or only self (1 member), or no friends: clear the name if it was auto-generated
        if (
          isAutoGenerated ||
          currentName === lastAutoGeneratedNameRef.current
        ) {
  ...
  }, [selectedMemberIds, friends, authData?.name, currentUserId]);
